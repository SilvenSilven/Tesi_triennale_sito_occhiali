...
const FIRST_FRAME = 28; // frames 000-027 removed
...
const BEATS: Beat[] = [
  ...
  },
  {
    ...
    align: "center",
    sizeClass: "text-5xl sm:text-7xl md:text-9xl",
    cta: { label: "Scegli il tuo modello →" },
  },
];
...
function Loader({ progress }: { progress: number }) {
  return (
    <motion.div
      className="fixed inset-0 z-40 flex flex-col items-center justify-center bg-sand"
      exit={{ opacity: 0 }}
      transition={{ duration: 0.6, ease: "easeInOut" }}
    >
      {/* Spinner */}
      <motion.div
        className="mb-8 h-10 w-10 rounded-full border-[3px] border-accent/20 border-t-accent"
        animate={{ rotate: 360 }}
        transition={{ duration: 1, repeat: Infinity, ease: "linear" }}
      />
      {/* Progress bar */}
      <div className="h-[3px] w-48 overflow-hidden rounded-full bg-warm/10">
        <motion.div
          className="h-full rounded-full bg-sun"
          style={{ width: `${progress}%` }}
        />
      </div>
      <p className="mt-4 font-body text-xs tracking-widest text-warm/40">
        CARICAMENTO
      </p>
    </motion.div>
  );
...
function BeatOverlay({
  ...
  return (
    ...
    >
      <h2
        className={`font-playfair font-bold tracking-tighter text-warm/90 leading-[0.95] ${sizeClass}`}
      >
        {title}
      </h2>
      <p className="mt-4 max-w-md text-base text-warm/55 sm:text-lg md:mt-6 md:max-w-xl md:text-xl">
        {subtitle}
      </p>
      {cta && (
        <a
          href="#pricing"
          className="pointer-events-auto mt-8 inline-block rounded-full bg-accent px-8 py-4 font-body text-sm font-semibold text-white shadow-[0_0_28px_#FF6B2B66] transition-all duration-300 hover:scale-[1.03] hover:shadow-[0_0_36px_#FFD16688] md:text-base"
        >
          {cta.label}
        </a>
      )}
...
function ScrollIndicator({
  ...
  return (
    <motion.div
      ...
      style={{ opacity }}
    >
      <span className="font-body text-xs tracking-[0.2em] text-warm/40 uppercase">
        Scorri per scoprire
      </span>
      <motion.svg
        width="20"
...
/* ── Main component ────────────────────────────────── */
export default function SolaraGlasses() {
  ...
  const frameIndex = useTransform(smoothProgress, (v: number) =>
    Math.min(163, Math.max(0, Math.floor(v * 163)))
  );
