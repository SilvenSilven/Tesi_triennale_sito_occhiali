...
async function test() {
  ...
  console.log('Pronte da pubblicare:', readyToPublish[0].total);
  ...
  const fakeReview = await sql\
    ...
    RETURNING id
  \;
  console.log('Nuovo ID:', fakeReview[0].id);

  console.log('Eseguo query cron...');
  ...
  \;
  console.log('Cron ha pubblicato:', cronResult.length);
  
  await sql\DELETE FROM reviews WHERE id = \\;
